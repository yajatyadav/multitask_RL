...
def normalize_libero_batch(batch, dataset_name: str):
    ...
    assert os.path.exists(norm_stats_folder), f"Norm stats folder {norm_stats_folder} does not exist"
...
def normalize_action_min_max(action, norm_stats):
    ...
    # scale everthing closer to 0 a bit so we don't get Nan log_probs
...
def unnormalize_action(action, norm_stats):
    assert action.dtype == np.float32
    action = np.array(action)
    action_norm_stats = norm_stats["actions"]
    mean, std = action_norm_stats["mean"], action_norm_stats["std"]
