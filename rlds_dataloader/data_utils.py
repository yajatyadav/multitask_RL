...
def binarize_gripper_actions(actions: tf.Tensor, normalize: bool) -> tf.Tensor:
    """
    ...
    If normalize is True, then 0=>-1 and 1=>1
    """
    ...
    binary_actions =  tf.scan(scan_fn, tf.range(tf.shape(actions)[0]), actions[-1], reverse=True)
    return 2.0 * binary_actions - 1.0 # map 0=>-1 and 1=>1
