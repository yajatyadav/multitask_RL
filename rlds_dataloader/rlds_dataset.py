...
from rlds_dataloader import traj_transforms
from rlds_dataloader.data_utils import (
    allocate_threads,
...
import rlds_dataloader.obs_transforms as obs_transforms
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    """
    REQUIRED_KEYS = {"observation", "action", "is_terminal", "reward"}
    ...
    def restructure(traj):
        ...
        # success = tf.math.reduce_any(traj["reward"] == 1)
        # extracts images, depth images and proprio from the "observation" dict
        traj_len = tf.shape(traj["action"])[0]
        ## TODO: need to add a "next_observation" DICT!
        ...
        traj = {
            ...
            "is_terminal": tf.cast(traj["is_terminal"], tf.bool),
    ...
    ## YY: failure filtering being done here, before the trajectories are restructured. Idle-step filtering handled by chunk_filter_fn, which is passed in under aux_kwargs
    if name in DATASETS_TO_FILTER_FAILURES:
        dataset = dataset.filter(lambda ep: tf.math.reduce_all(tf.strings.regex_full_match(ep["traj_metadata"]["episode_metadata"]["file_path"], ".*success.*"))) # an episode is a "success" if each step's file_path contains "success" (should be the same file path)
    
    # success_count = 0
    # for _ in tqdm(dataset, desc="Iterating over dataset to verify success count"):
        # success_count += 1
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    ## YY: the success field was added to each traj the restructure function, we can filter on it here
    # dataset = dataset.filter(lambda x: tf.math.reduce_any(x["success"]))
    
    # count = 0
    # for _ in tqdm(dataset):
        # count += 1
    # print("AFTER FILTERING, THIS DATASET HAS NUM_TRAJECTORIES", count)
...
def apply_per_dataset_frame_transforms(
    ...
    image_transform_fn: Optional[Callable] = None,
):
    """
    ...
    if image_transform_fn:
        dataset = dataset.map(image_transform_fn)
    if chunk_filter_fn:
