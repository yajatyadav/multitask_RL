...
def chunk_act_obs(traj: Dict, window_size: int, future_action_window_size: int = 0) -> Dict:
    ...
    chunk_indices = tf.broadcast_to(tf.range(0, window_size), [traj_len, window_size]) + tf.broadcast_to(
    ...
    traj["observation"] = tf.nest.map_structure(lambda x: tf.gather(x, capped_chunk_indices), traj["observation"])
    traj["action"] = tf.gather(traj["action"], capped_action_chunk_indices)


    # indicates whether an entire observation is padding
    traj["observation"]["pad_mask"] = chunk_indices < traj_len
